{{-- resources/views/User/promo/show.blade.php --}}
<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <title>{{ $promo->name ?? 'Detail Promo' }} | Samafitro</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    {{-- Bootstrap & Tailwind --}}
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
    <script src="https://cdn.tailwindcss.com"></script>

    <style>
        body { background-color: #1f1f1f; color: #fff; }
        .promo-tag {
            position: absolute;
            top: 12px;
            left: 12px;
            background: #d32f2f;
            color: #fff;
            padding: .4rem .9rem;
            border-radius: 8px;
            font-weight: 700;
            z-index: 5;
        }
        .promo-card {
            background: linear-gradient(180deg, rgba(255,255,255,0.03), rgba(0,0,0,0.03));
            border-radius: 14px;
            padding: 20px;
        }
        .info-box {
            background: rgba(255,255,255,0.03);
            border-radius: 12px;
            padding: 16px;
        }
        .badge-cta {
            background: #d32f2f;
            color: #fff;
            padding: .35rem .6rem;
            border-radius: 999px;
            font-weight: 700;
        }
        .badge-time {
            display: inline-flex;
            align-items: center;
            justify-content: center;
            font-size: 1rem;
            font-weight: 600;
            padding: 0.4rem 0.8rem;
            min-width: 110px;
            text-align: center;
            line-height: 1.2;
        }
        /* Floating WA button style (kanan bawah) */
        .floating-wa { 
            position: fixed;
            bottom: 20px;
            right: 20px;
            z-index: 999;
            width: 55px;
            height: 55px;
            border-radius: 50%;
            box-shadow: 0 4px 8px rgba(0, 0, 0, 0.3);
            transition: transform 0.3s ease, box-shadow 0.3s ease;
        }
        .floating-wa img {
            width: 100%;
            height: 100%;
            object-fit: cover;
            border-radius: 50%;
        }
        .floating-wa:hover {
            transform: scale(1.1);
            box-shadow: 0 6px 14px rgba(0, 255, 0, 0.5);
        }
        /* Responsive tweaks */
        @media (max-width: 767px) {
            .promo-hero-inner { flex-direction: column; }
            .promo-image-wrapper { width: 100% !important; max-width: 100% !important; }
            .promo-image-wrapper img { height: auto !important; }
            .promo-card { padding: 15px; }
        }
    </style>
</head>
<body class="bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white font-sans">

    {{-- Navbar (versi responsive) --}}
    <nav x-data="{ open: false }" class="bg-gray-950 shadow fixed top-0 left-0 w-full z-50">
        <div class="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
            <div class="flex items-center space-x-3">
                <img src="{{ asset('images/logo-samafitro.png') }}" alt="Samafitro" class="h-12 md:h-16 w-auto max-h-14">
            </div>
            <ul class="hidden md:flex flex-wrap space-x-4 lg:space-x-8 text-xs md:text-sm font-medium text-white">
                <li><a href="{{ route('dashboard') }}" class="hover:text-blue-400">Beranda</a></li>
                <li><a href="#" class="hover:text-blue-400">Produk</a></li>
                <li><a href="{{ route('promo.index') }}" class="hover:text-blue-400">Promo</a></li>
                <li><a href="{{ route('article.index') }}" class="hover:text-blue-400">Artikel & Berita</a></li>
                <li><a href="#" class="hover:text-blue-400">Hubungi Kami</a></li>
            </ul>
            <div class="hidden md:flex space-x-4">
                <a href="#"><img src="/images/cart.png" alt="Cart" class="h-6 w-6 hover:opacity-80" /></a>
                <a href="{{ route('User.profile.index') }}"><img src="/images/profile.png" alt="User" class="h-6 w-6 hover:opacity-80" /></a>
            </div>
            {{-- Mobile Hamburger --}}
            <div class="md:hidden flex items-center space-x-4">
                <a href="#"><img src="/images/cart.png" alt="Cart" class="h-6 w-6 hover:opacity-80" /></a>
                <a href="{{ route('User.profile.index') }}"><img src="/images/profile.png" alt="User" class="h-6 w-6 hover:opacity-80" /></a>
                <button @click="open = !open" class="text-white focus:outline-none ml-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none"
                        viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>
                    </svg>
                </button>
            </div>
        </div>
        {{-- Mobile Menu --}}
        <div x-show="open" x-transition
            class="md:hidden absolute top-full left-0 w-full bg-gray-950 border-t border-gray-800">
            <ul class="flex flex-col space-y-3 px-6 py-4 text-white text-sm">
                <li><a href="{{ route('dashboard') }}" class="hover:text-blue-400">Beranda</a></li>
                <li><a href="#" class="hover:text-blue-400">Produk</a></li>
                <li><a href="{{ route('promo.index') }}" class="hover:text-blue-400">Promo</a></li>
                <li><a href="{{ route('article.index') }}" class="hover:text-blue-400">Artikel & Berita</a></li>
                <li><a href="#" class="hover:text-blue-400">Hubungi Kami</a></li>
            </ul>
        </div>
    </nav>
    <div class="pt-20"></div>

    {{-- Hero / detail --}}
    <section class="max-w-7xl mx-auto px-4 sm:px-6 py-8">
            {{-- Tombol kembali (ikon panah) --}}
    <div class="mb-4">
        <a href="{{ route('promo.index') }}" class="inline-flex items-center text-white hover:text-blue-400">
            <i class="fas fa-arrow-left mr-2"></i> Kembali
        </a>
    </div>
    <div class="promo-card">
        <div class="d-flex flex-wrap promo-hero-inner gap-4">
        <div class="promo-card">
            <div class="d-flex flex-wrap promo-hero-inner gap-4">

                {{-- Kiri: Gambar --}}
                <div class="position-relative promo-image-wrapper" style="flex: 0 0 46%; max-width:46%;">
                    @if($promo->image && \Storage::disk('public')->exists($promo->image))
                        <div class="promo-tag">
                            Diskon {{ $promo->discount ?? '30%' }}
                            <div style="font-size:.7rem; font-weight:400;">Brand {{ $promo->vendor ?? '' }}</div>
                        </div>
                        <img src="{{ asset('storage/' . $promo->image) }}" alt="{{ $promo->name }}"
                            class="img-fluid rounded-3 w-100"
                            style="object-fit:cover; height:320px;">
                    @else
                        <div class="promo-tag">Diskon {{ $promo->discount ?? '30%' }}</div>
                        <div class="bg-gray-800 rounded-3 d-flex align-items-center justify-content-center" style="height:320px;">
                            <i class="fas fa-image fa-3x text-muted"></i>
                        </div>
                    @endif

                    <div class="mt-3 d-flex flex-wrap gap-2">
                        <span class="badge-cta">{{ $promo->name }}</span>
                        <span class="badge bg-secondary badge-time">
                            {{ \Carbon\Carbon::parse($promo->periode)->diffForHumans() ?? '' }}
                        </span>
                    </div>
                </div>

                {{-- Kanan: Detail --}}
                <div style="flex: 1; min-width: 280px;">
                    <h1 class="display-6 fw-bold">{{ $promo->name }}</h1>

                    <p class="lead text-muted" style="font-size:1.05rem;">
                        {!! nl2br(e($promo->description ?? 'Deskripsi promo belum tersedia.')) !!}
                    </p>

                    <div class="row mt-4 g-3">
                        <div class="col-md-6">
                            <div class="info-box">
                                <h6 class="fw-semibold">Syarat dan Ketentuan</h6>
                                <ul class="mb-0 small">
                                    @if($promo->terms)
                                        @foreach(explode("\n", trim($promo->terms)) as $term)
                                            @if(trim($term) !== '')
                                                <li>{{ trim($term) }}</li>
                                            @endif
                                        @endforeach
                                    @else
                                        <li>-</li>
                                    @endif
                                </ul>
                            </div>
                        </div>

                        <div class="col-md-6">
                            <div class="info-box">
                                <h6 class="fw-semibold">Informasi Detail</h6>
                                <p class="small mb-0">
                                    <strong>Vendor:</strong> {{ $promo->vendor ?? '-' }}<br>
                                    <strong>Label:</strong> {{ $promo->label ?? '-' }}<br>
                                    <strong>Periode:</strong> {{ \Carbon\Carbon::parse($promo->periode)->format('d M Y') ?? '-' }}<br>
                                    <strong>Diskon:</strong> {{ $promo->discount ?? '-' }}
                                </p>
                            </div>
                        </div>
                    </div>

                    <div class="mt-4 d-flex flex-wrap gap-3">
                        @php
                            $waNumber = '6281234567890';
                            $waText = urlencode("Halo, saya tertarik promo: {$promo->name} (ID: {$promo->id}). Mohon info lebih lanjut.");
                        @endphp

                        <a href="https://wa.me/{{ $waNumber }}?text={{ $waText }}" target="_blank"
                            class="btn btn-success btn-lg d-inline-flex align-items-center">
                            <img src="{{ asset('images/cart.png') }}" alt="Cart"
                                style="width: 20px; height: 20px; margin-right: 8px;">
                            Pesan Sekarang
                        </a>
                    </div>
                </div>
            </div>
        </div>
    </section>

    {{-- Floating WA button --}}
    <a href="https://wa.me/6281234567890" target="_blank" class="floating-wa" title="Chat WhatsApp">
        <img src="{{ asset('images/buttonwa.png') }}" alt="WhatsApp">
    </a>

    {{-- Footer --}}
<footer class="bg-gray-900 py-10 text-white text-sm">
    <div class="max-w-7xl mx-auto px-4 flex flex-col md:flex-row gap-10">
        <div class="flex-shrink-0">
            <img src="{{ asset('images/logo-samafitro.png') }}" alt="Samafitro Logo" class="h-20 sm:h-24">
        </div>
        <div>
            <h3 class="text-lg font-semibold mb-4">Social Media Kami :</h3>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/email.png') }}" alt="Email" class="h-5 w-5">
                    <span>samafitro_bdg@samafitro.co.id</span>
                </div>

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/instagram.png') }}" alt="Instagram" class="h-5 w-5">
                    <span>@samafitro_bandung</span>
                </div>

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/youtube.png') }}" alt="YouTube" class="h-5 w-5">
                    <span>Samafitro Bandung</span>
                </div>

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/tokopedia.png') }}" alt="Store" class="h-5 w-5">
                    <span>Official Store</span>
                </div>

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/facebook.png') }}" alt="Facebook" class="h-5 w-5">
                    <span>@SamafitroBandung</span>
                </div>

                <div class="flex items-center gap-2">
                    <img src="{{ asset('images/tiktok.png') }}" alt="TikTok" class="h-5 w-5">
                    <span>@samafitro.bandung</span>
                </div>
            </div>
        </div>
    </div>
    <div class="mt-8 border-t border-gray-700 pt-4 text-center">
        <p class="flex items-center justify-center gap-2">
            <span>Copyright</span>
            <img src="{{ asset('images/ccopyright.png') }}" alt="Copyright" class="h-4 w-4">
            <span>2025 Cabang PT Samafitro Bandung</span>
        </p>
        <p>Oleh Tim Developer Kami</p>
    </div>
</footer>

    {{-- Alpine.js --}}
    <script src="//unpkg.com/alpinejs" defer></script>
</body>
</html>
